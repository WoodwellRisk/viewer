...
const SearchBox = () => {
  ...
  const handleSearch = (event) => {
    ...
    if (numResults < limit) {
      let additionalResults = searchArray
        .filter((place) => {
          return (
            !place[0]['name'].startsWith(query) &&
            place[0]['name'].toLowerCase().includes(query.toLowerCase())
          );
        })
    ...
    setResults(
      tempResults.map((place) => [
        place[0]['name'],
        place[0]['search'],
        place[0]['search'] == 'cities' ? place[0]['coords'] : place[0]['bbox'],
      ])
    );
  };
