...
const SearchLayer = ({ id, source, color, minZoom = 0, maxZoom = 8.9, opacity = 1, type }) => {
      ...
      if (!map.getLayer(layerId)) {
        ...
        if (type == 'line') {
          ...
          if (!map.style.stylesheet.glyphs) {
            console.log(
              'Please specify a glyphs object in the <Map /> component in order to use text labels.'
            );
            // return
          }
          ...
          // if type is not in ['cirle', 'line'], return null
          return null;
        }
