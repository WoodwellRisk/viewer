...
const JustAccess = ({ theme }) => {
        ...
        try {
          ...
          map.on('click', 'just-access-fill', (e) => {
            ...
            if (pointFeatures.length == 0 && polygonFeatures.length > 0) {
              ...
              popup.setLngLat(e.lngLat).setHTML(htmlString).setMaxWidth(500).addTo(map);
        ...
        try {
          ...
          map.addLayer({
            ...
            paint: {
              ...
              // 'circle-opacity': 0.3,
              'circle-opacity': ['case', ['boolean', ['feature-state', 'hover'], false], 0.5, 0.3],
  ...
  useEffect(() => {
    ...
    updatePaintProperty(map, 'just-access-points', 'circle-stroke-color', theme.rawColors.primary);
  }, [theme]);
