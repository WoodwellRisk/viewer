...
const Router = () => {
  ...
  const getInitialCategoryAndLayer = useCallback((url) => {
    ...
    if (
      tempCategory != null &&
      typeof tempCategory == 'string' &&
      categories.includes(tempCategory.replace('-', ' '))
    ) {
      ...
      if (
        tempLayer != null &&
        typeof tempLayer == 'string' &&
        associatedRisks[initialCategory].includes(tempLayer)
      ) {
  ...
  useEffect(() => {
    ...
    router.replace(
      `${pathname}?category=${url.searchParams.get('category')}&layer=${url.searchParams.get(
        'layer'
      )}&zoom=${url.searchParams.get('zoom')}&center=${url.searchParams.get('center')}`
    );
    // prevent back button
