...
const DownloadBarData = ({ data, fileType }) => {
    ...
    const region = regionData.value['coordinates'];
    ...
    const handleDownloadClick = () => {
        ...
        if(fileType == 'JSON') {
            let json = {
                ...
                'accessed': now,
                'variable': `${riskThemeLabels[variable]}`,
                'crop': `${crop}`,
                'band': `${band}`,
                'extent': {
                    'lat': [minLat, maxLat],
            ...
            saveAs(blob, `${variable}${variable.startsWith('cf') ? '-' + crop : ''}-${band}.json`.replaceAll('_', '-'))
            ...
            csv += `# Accessed: ${now}\n`;
            csv += `# Variable: ${riskThemeLabels[variable]}\n`
            ...
            saveAs(blob, `${variable}${variable.startsWith('cf') ? '-' + crop : ''}-${band}.csv`.replaceAll('_', '-'))
        } else {
