...
const DownloadTimeseriesData = ({ data, fileType }) => {
  ...
  const handleDownloadClick = () => {
    ...
    if (fileType == 'JSON') {
      ...
      if (risk.startsWith('cf')) {
        jsonString = jsonString.replaceAll('"crop"', '  "crop"');
      }
      ...
      saveAs(
        blob,
        `${risk}${risk.startsWith('cf') ? '-' + crop : ''}-timeseries.json`.replaceAll('_', '-')
      );
      ...
      saveAs(
        blob,
        `${risk}${risk.startsWith('cf') ? '-' + crop : ''}-timeseries.csv`.replaceAll('_', '-')
      );
    } else {
