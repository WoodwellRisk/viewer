...
const Layers = () => {
  ...
  const handleCropChange = useCallback((event) => {
    ...
    if (Object.keys(cropOptions).includes(crop)) setCrop(crop);
  });
      ...
      <Box sx={sx.group}>
        <Box sx={{ mt: -3 }} className="risk-theme-container">
          <Box as="h2" sx={sx.h2} className="risk-theme-title">
            Climate risks{' '}
            <Info>
              Choose a climate risk category to see a list of related data layers. Some layers can
              be found in multiple categories (e.g.{' '}
              <Text sx={{ fontStyle: 'italic' }}>temperature and health</Text> or{' '}
              <Text sx={{ fontStyle: 'italic' }}>wildfire and energy</Text>).
            </Info>
          </Box>
          ...
          <Box as="h3" sx={sx.h3} className="risk-title">
            Layers{' '}
            <Info>
              Several layers on this map were created by aggregating climate model output not by
              year, but by warming level. To learn more about this approach, please see our{' '}
              <Link
                href="https://woodwellrisk.github.io/tools/warming-levels/#evaluating-warming-level-calculations"
                target="_blank"
              >
                methodology website
              </Link>
              ...
              <Box sx={{ ...sx.label, mt: [4], width: '100%' }}>
                ...
                <Slider
                  sx={{
                    mt: [3],
                    mb: [2],
                    width: risk.startsWith('tc') || risk.startsWith('cf') ? '150px' : '175px',
                ...
                >
                  {risk.startsWith('tc') || risk.startsWith('cf')
                    ? riskOptions[risk].bandLabels[bandIndex]
                    : risk == 'lsp' || risk == 'pm25'
                      ? riskOptions[risk].bands[bandIndex].toFixed(0)
